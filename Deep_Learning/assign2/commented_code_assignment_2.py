...
"""   
"""
Dylan Fontaine, Dustin Fontaine, Michael Cho, Annie Didier
Problem 2: Using the defaults for all other settings and 50 epochs, we changed cfg.xferlearning = 2 
Problem 3: Step1: 
            We adjusted the values of cfg.vgglearning and cfg.xferlearning to find the optimal combination. 
            Combinations used are given in the report
            Step2:
            Using cfg.vgglearning = 2 and cfg.xferlearning = -1, we adjusted the image size to 
            imsize = (n,) * 2, where n = 32, 64, 96   
Problem 4: We adjusted cfg.fclayers = 0,2,4 and kept all other values at their defaults. 
Problem 5: To examine the effect of the different data augmentation strategies, 
            we set each variable in the ImageDataGenerator function to false or 0
            and turned each feature "on" one at a time. Numeric values are as 
            given in the report. 
            
            With all other variables set to default, we adjusted cfg.fcdropout
            with values as given in the report.
            
            To examin the effects of using different color representations we
            changed cfg.hsv to TRUE and used default values for all other variables. 
            
Problem 6: #a Using the default settings of 2 VGG layers and no transfer learning, 
             we examined the effects of different sized filters in the VGG net definition
             section. For every layer that was used we adjusted the filter size in the 
             call to Convolution2D. We tried combinations of 2x2, 3x3, and 4x4 filters with 
             cfg.vgglayers =2,3,5. For consistency, we kept all other parameters at their 
             defaults and kept transfer learning off. 
            #b optimizer = Adam(), SGD(), RMSprop(), or Adagrad() with default settings elsewhere
...
if cfg.vgglayers >= 3: # Block 3
    x = Convolution2D(256, (4, 4), activation='relu', padding='same', name='block3_conv1')(x)
    x = Convolution2D(256, (4, 4), activation='relu', padding='same', name='block3_conv2')(x)
    x = Convolution2D(256, (4, 4), activation='relu', padding='same', name='block3_conv3')(x)
