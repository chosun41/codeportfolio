...
for i in range(1000): # Do not change this, we will compare performance at 1000 epochs
    ...
    dW3 = (L3 - T) * np.where(L3*(1 - L3)<1e-2, 1e-2, L3*(1 - L3))
    dW2 = W3.T.dot(dW3)*np.where(L2*(1-L2)<1e-2, 1e-2, L2*(1-L2)) 
    dW1 = W2.T.dot(dW2)*np.where(L1*(1-L1)<1e-2, 1e-2,L1*(1-L1)) 
    # Update
    ...
    if i % 10 == 0:
        ...
        plt.savefig(os.path.join('actvgrad/train', 'train-%05d.png' % i))
        plt.show(), plt.close()
...
plt.suptitle("Activation and Gradient improvements")
plt.savefig(os.path.join('actvgrad', 'actvgrad.png'))
