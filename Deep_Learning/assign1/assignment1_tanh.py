...
for i in range(1000): # Do not change this, we will compare performance at 1000 epochs
    # Forward pass (changed sigmoid to np.tanh)
    L1 = np.tanh(W1.dot(X))
    L2 = np.tanh(W2.dot(L1))
    L3 = np.tanh(W3.dot(L2))
    # Backward pass (changed the backward pass from the sigmoid derivative to tanh derivative
    #  which is 1-tan^2(x))
    dW3 = (L3 - T) * (1 - L3**2)
    ...
    if i % 10 == 0:
        ...
        plt.savefig(os.path.join('tanh/train', 'train-%05d.png' % i))
        plt.show(), plt.close()
...
plt.suptitle("Tanh")
plt.savefig(os.path.join('tanh', 'tanh.png'))