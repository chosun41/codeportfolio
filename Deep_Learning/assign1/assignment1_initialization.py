...
low1 = -4*np.sqrt(6.0/(784 + 256)) # use 4 for sigmoid, 1 for tanh activation 
high1 = 4*np.sqrt(6.0/(784 + 256))
low2 = -4*np.sqrt(6.0/(256 + 128)) # use 4 for sigmoid, 1 for tanh activation 
high2 = 4*np.sqrt(6.0/(256 + 128))
low3 = -4*np.sqrt(6.0/(128 + 10)) # use 4 for sigmoid, 1 for tanh activation 
high3= 4*np.sqrt(6.0/(128 + 10))

# list of different weight initializations
# [-1,1], [-4,4], random normal/sqrt fan in, sd 1, he initalized, glorot uniform
init=[[2*np.random.rand(784, 256).astype('float32').T - 1,
      2*np.random.rand(256, 128).astype('float32').T - 1,
      2*np.random.rand(128,  10).astype('float32').T - 1],
     [8*np.random.rand(784, 256).astype('float32').T - 4,
      8*np.random.rand(256, 128).astype('float32').T - 4,
      8*np.random.rand(128,  10).astype('float32').T - 4],
      [np.random.randn(784,256).astype('float32').T/np.sqrt(784).T,
       np.random.randn(256,128).astype('float32').T/np.sqrt(256).T,
       np.random.randn(128,10).astype('float32').T/np.sqrt(128).T],
      [np.random.normal(0,2/784, size=(784,256)).astype('float32').T ,
       np.random.normal(0,2/256, size=(256,128)).astype('float32').T ,
       np.random.normal(0,2/128, size=(128,10)).astype('float32').T ],
      [np.random.uniform(low1, high1, size=(784,256)).astype('float32').T ,
       np.random.uniform(low2, high2, size=(256,128)).astype('float32').T ,
       np.random.uniform(low3, high3, size=(128,10)).astype('float32').T ]]
     
lr = 1e-5              # Learning rate, decrease if optimization isn't working
def sigmoid(x): return 1.0/(1.0 + np.e**-x)

# for loop to cycle through the different weight initializations
for indx, value in enumerate(init):
    W1 = value[0]
    W2 = value[1]
    W3 = value[2]
    ...
    for i in range(200): # Do not change this, we will compare performance at 1000 epochs
        ...
        if i % 10 == 0:
            ...
            plt.savefig(os.path.join('init/train', 'init%03d-%05d.png' % (indx,i)))            
            plt.show(), plt.close()
    ...
    plt.savefig(os.path.join('init', 'init%03d.png' % indx))