...
X = X.astype('float32').reshape((len(X), -1))
...
# normalize across training samples for each pixel value
...
#np.where(np.sum(np.exp(z), axis = 0)==0, 0.001,np.sum(np.exp(z), axis = 0))

crossentrps, accuracies, hw1, hw2, hw3, ma = [], [], [], [], [], []

for i in range(1000): # Do not change this, we will compare performance at 1000 epochs
    # Forward pass (normalize sigmoid by fan in)
    L1 = relu(W1.dot(X))
    L2 = np.tanh(W2.dot(L1))
    L3 = softmax(W3.dot(L2))
    
    # Backward pass (clip gradient at 0.01 to recover stay in active region)
    dW3 = (L3 - T) 
    dW2 = W3.T.dot(dW3)*np.where((1-L2**2)<0.01,0.01,(1-L2**2))
    dW1 = np.where(W1.dot(X)<=0,0.01*W1.dot(X),W2.T.dot(dW2))
    # Update
...
#    ce = np.sum(-T*np.log(L3)-(1-T)*np.log(1-L3))/len(T.T)
    ...
    if i % 10 == 0:
        ...
        plt.savefig(os.path.join('final/train', 'train-%05d.png' % i))
        plt.show(), plt.close()
...
plt.savefig(os.path.join('final', 'actvgrad.png'))
