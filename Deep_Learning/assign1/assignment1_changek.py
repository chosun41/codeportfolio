...
# klst is 
klst=[1e-2,1e-1,1e0,1e1, 1e2]
def sigmoid(k,x): return 1.0/(1.0 + np.e**-(k*x))

# created for loop for each k
for k in klst:
    ...
    for i in range(200): # Do not change this, we will compare performance at 1000 epochs
        # Forward pass
        L1 = sigmoid(k, W1.dot(X))
        L2 = sigmoid(k, W2.dot(L1))
        L3 = sigmoid(k, W3.dot(L2))
        # Backward pass
        dW3 = (L3 - T) * L3*(1 - L3)
        ...
        if i % 10 == 0:
            ...
            plt.savefig(os.path.join('k/train', '%0.3f-%05d.png' % (k,i)))            
            plt.show(), plt.close()
    ...
    plt.suptitle("k=%0.4f" % (k))
    plt.savefig(os.path.join('k', 'k%0.4f.png' % k))
